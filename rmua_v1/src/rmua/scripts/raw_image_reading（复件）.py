...
class YoloV5Node:
    def __init__(self):
        ...
        self.landing_launch_file = rospy.get_param('~landing_launch_file', '/home/amov/rmua_v1/src/rmua/launch/landing.launch')
        ...
        self.landing_image_sub = rospy.Subscriber("/iris/usb_cam/image_raw", Image, self.landing_image_callback, queue_size=1)
    ...
    def landing_image_callback(self, data):
        """降落相机图像回调，用于降落阶段的目标检测"""
        try:
            self.landing_image = self.bridge.imgmsg_to_cv2(data, "bgr8")
            
            # 如果处于降落模式，直接在这里处理降落相机图像
    ...
    def local_position_callback(self, data):
        try:
            ...
            # 检查是否需要开始降落
            if self.target_points[self.CURRENT_FLAG]["label"] == "Land" and self.body_vel < 0.1:
    ...
    def callback(self, data):
        ...
        try:
            ...
            # 处理不同目标类型
            if self.CURRENT_FLAG < len(self.target_points) and self.target_points[self.CURRENT_FLAG]["label"] == "square":
                # 处理正方形目标
                ...
                if square_contours and depth_meters is not None:
                    ...
                    if x_local is not None:
                        ...
                        if self.CURRENT_FLAG < len(self.target_points):
                            ...
                            if target_distance < 2 and not self.goal_published:
            ...
            elif self.yaw_rate < 0.01 and self.CURRENT_FLAG < len(self.target_points) and  self.target_points[self.CURRENT_FLAG]["label"] == "special_circle":
                ...
                if depth_meters is not None:
                    ...
                    z_cam = depth_meters
                    x_body, y_body, z_body = self.transform_camera_to_body(x_cam, y_cam, z_cam)
                    x_actual_local, y_actual_local, z_actual_local = self.transform_body_to_local(x_body, y_body, z_body, self.color_timestamp)
                    
                    # 检查是否获得了有效位置
                    if x_actual_local is not None:
                        
                        self.handle_circle_flight(x_body, y_body, z_body, x_actual_local, y_actual_local, z_actual_local)
    ...
    def process_spcial_circle(self, results, cv_image):
        return
    ...
    def handle_circle_flight(self, x_body, y_body, z_body, x_actual_local, y_actual_local, z_actual_local):
        ...
        x_front_local, y_front_local, z_front_local = self.transform_body_to_local(x_front_body, y_front_body, z_front_body, self.color_timestamp)
        ...
        x_through_local, y_through_local, z_through_local = self.transform_body_to_local(x_through_body, y_through_body, z_through_body, self.color_timestamp)
        ...
        # 如果已经发送了前置点，检查是否已到达前置点
            ...
            if self.last_circle_position:
                ...
                # 如果已达前置点且圆位置稳定，发送穿过点
                if dist_to_front < 0.1 and pos_diff < 0.1:
    ...
    def transform_body_to_local(self, x_body, y_body, z_body, frame_timestamp):
        ...
        if min_time_diff > self.timestamp_threshold:
            rospy.logwarn(f"Time difference ({min_time_diff:.3f}s) exceeds threshold ({self.timestamp_threshold:.3f}s)")
            return None, None, None, None
