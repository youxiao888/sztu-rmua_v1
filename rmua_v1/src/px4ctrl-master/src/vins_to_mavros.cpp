...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()) {
        ...
        vision.pose.position.x = -p_mav[1];
        vision.pose.position.y = p_mav[0];
        ...
        ROS_INFO("\nposition:\n   x: %.18f\n   y: %.18f\n   z: %.18f\norientation:\n   x: %.18f\n   y: %.18f\n   z: %.18f\n   w: %.18f", \
        vision.pose.position.x,vision.pose.position.y,vision.pose.position.z,-q_mav.x(),-q_mav.y(),-q_mav.z(),-q_mav.w());
